...
for fp in files:
    if os.path.exists(fp):
        ...
        # Replace the old text if it exists to avoid duplicates
        content = content.replace(hint_text, "")
        
        # In these files, there is a label like:
        # <label className="block text-sm font-bold text-gray-700 mb-4">📷 {t('safetyReports.image')}</label>
        # or {t('workPermits.image')} or {t('qualityReports.image')} or {t('violations.images')}
        ...
        # Actually, let's look for `<div className="flex gap-4 mb-4">` which is right after the label.
        # But wait, there might be multiple of these.
